...
export function ReportDateFilter({value,onChange,minDate,maxDate}:Props){
 const [open,setOpen]=useState(false); const [loadingAll,setLoadingAll]=useState(false); const today=useMemo(()=>new Date(),[]); const anchor=value.end?fromIso(value.end):today;
 ...
 return <div className="relative"><button type="button" onClick={()=>setOpen(!open)} className="min-h-9 border border-slate-300 bg-white px-3 text-xs font-bold">الفترة: {label}⌄</button>{open&&<div className="absolute right-0 top-10 z-50 w-[min(92vw,360px)] border border-slate-300 bg-white shadow-xl"><div className="border-b border-slate-100 px-4 py-3 text-xs font-bold text-slate-700">الفترة الزمنية</div>
