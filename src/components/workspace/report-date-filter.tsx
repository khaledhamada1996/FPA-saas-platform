...
export function ReportDateFilter({ value, onChange, minDate, maxDate }: Props) {
  ...
  const anchor = value.end ? fromIso(value.end) : today;
  const label = value.start && value.end ? `${value.start} — ${value.end}` : "اختر الفترة";

  const apply = (start: Date, end: Date) => {
    const range = clampRange({ start: localIso(start), end: localIso(end) }, minDate, maxDate);
    onChange(range);
    setOpen(false);
  };

  const all = () => {
    if (minDate && maxDate) onChange({ start: minDate, end: maxDate });
    else if (minDate) onChange({ start: minDate, end: localIso(today) });
  ...
  return (
    <div className="relative">
      <button type="button" onClick={() => setOpen(!open)} className="min-h-9 border border-slate-300 bg-white px-3 text-xs font-bold">
        الفترة: {label}⌄
      </button>
      {open && (
        <div className="absolute right-0 top-10 z-50 w-[min(92vw,360px)] border border-slate-300 bg-white shadow-xl">
          <div className="border-b border-slate-100 px-4 py-3 text-xs font-bold text-slate-700">الفترة الزمنية</div>
          <button type="button" onClick={() => apply(today, today)} className="block w-full border-b px-4 py-3 text-right text-xs">اليوم</button>
          ...
          <button type="button" onClick={() => { const end = addDays(startOfWeek(today), -1); apply(startOfWeek(end), end); }} className="block w-full border-b px-4 py-3 text-right text-xs">الأسبوع السابق</button>
          ...
          <button type="button" onClick={all} className="block w-full border-b px-4 py-3 text-right text-xs font-bold">كل البيانات</button>
          <div className="bg-slate-50 p-3">
            <div className="mb-2 text-[11px] font-bold">تاريخ مخصص</div>
            <div className="grid gap-2">
              <input type="date" value={value.start} min={minDate} max={value.end || maxDate} onChange={e => onChange({ ...value, start: e.target.value })} className="min-h-9 border bg-white px-2 text-xs" />
              <input type="date" value={value.end} min={value.start || minDate} max={maxDate} onChange={e => onChange({ ...value, end: e.target.value })} className="min-h-9 border bg-white px-2 text-xs" />
              <button type="button" onClick={() => setOpen(false)} className="min-h-9 bg-slate-950 text-xs font-bold text-white">تطبيق</button>
            </div>
          </div>
        </div>
      )}
