...
export default function StartPage() {
  ...
  useEffect(() => {
    ...
    async function loadCompanies() {
      ...
      setExpanded(new Set(nextCompanies.filter((company) => company.children_count > 0).map((company) => company.id)));
      setState("companies");
    }
  ...
  function addSubsidiary(company: Company) {
    router.push(`/start/new?parent=${encodeURIComponent(company.id)}`);
  }
  ...
  async function deleteCompany() {
    ...
    try {
      ...
      if (window.sessionStorage.getItem("activeOrganizationId") === deleteTarget.id) window.sessionStorage.removeItem("activeOrganizationId");
  ...
  if (state === "loading") return <CenteredState text="جارٍ تحميل مساحة العمل…" />;
      ...
      <main className="min-h-screen bg-[#f7f8fa] px-4 py-8 text-[#172033] sm:px-6" dir="rtl">
        <div className="mx-auto flex min-h-[calc(100dvh-4rem)] w-full max-w-5xl items-center justify-center">
          <section className="w-full max-w-2xl rounded-3xl border border-slate-200 bg-white p-6 shadow-sm sm:p-10">
            <div className="flex flex-col gap-6 sm:flex-row sm:items-start sm:justify-between"><div><p className="text-xs font-bold tracking-[0.16em] text-slate-400">WELCOME TO FP&A</p><h1 className="mt-3 text-2xl font-bold tracking-tight text-slate-950 sm:text-3xl">لننشئ أول شركة لك</h1><p className="mt-3 max-w-xl text-sm leading-7 text-slate-500 sm:text-base">لن نطلب منك معلومات الشركة في كل تسجيل دخول. هذه الشاشة تظهر فقط عندما لا توجد شركة مرتبطة بحسابك.</p></div><span className="shrink-0 rounded-full bg-slate-100 px-3 py-1.5 text-xs font-semibold text-slate-600">لا توجد شركات</span></div>
  ...
  return (
    <main className="min-h-screen bg-[#f7f8fa] px-4 py-6 text-[#172033] sm:px-6 lg:px-10" dir="rtl">
      ...
      <section className="mx-auto w-full max-w-7xl py-8 sm:py-10 lg:py-14">
        <div className="max-w-3xl"><p className="text-xs font-bold tracking-[0.16em] text-slate-400">YOUR ORGANIZATION GROUP</p><h2 className="mt-3 text-3xl font-bold tracking-tight text-slate-950 sm:text-4xl">هيكل الشركات</h2><p className="mt-3 text-sm leading-7 text-slate-500 sm:text-base">أدير المجموعة من هنا: الشركة الأم، الشركات التابعة، ثم الشركات التشغيلية تحتها. اضغط السهم لفتح أو إغلاق أي فرع.</p></div>
        ...
        <div className="mt-8 rounded-3xl border border-slate-200 bg-white p-5 shadow-sm sm:p-7">
          ...
          <div className="mt-7 space-y-3">
            {companyTree.map((node) => <CompanyTreeNode key={node.company.id} node={node} depth={0} expanded={expanded} onToggle={toggleCompany} onEnter={enterCompany} onAddSubsidiary={addSubsidiary} onDelete={openDelete} />)}
          </div>
        </div>

        <div className="mt-8 rounded-2xl border border-slate-200 bg-white p-5 text-sm leading-7 text-slate-500 sm:p-6"><span className="font-bold text-slate-900">هيكل المجموعة:</span> يمكن بناء مستويات متعددة مثل شركة قابضة ← شركة تابعة ← شركة تشغيلية. إضافة شركة تابعة تمر بتحقق الصلاحية من قاعدة البيانات، وليس من واجهة المستخدم فقط.</div>
      </section>
      ...
      {deleteTarget && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/50 p-4" role="dialog" aria-modal="true" aria-labelledby="delete-company-title">
          <div className="w-full max-w-lg rounded-3xl border border-slate-200 bg-white p-6 shadow-2xl sm:p-8">
            <p className="text-xs font-bold tracking-[0.14em] text-red-500">DANGER ZONE</p><h2 id="delete-company-title" className="mt-2 text-2xl font-bold text-slate-950">حذف الشركة نهائيًا</h2><p className="mt-3 text-sm leading-7 text-slate-600">هذا الإجراء يحذف مساحة الشركة وبياناتها المرتبطة بها ولا ينبغي تنفيذه إلا إذا كنت متأكدًا تمامًا.</p>
            ...
            <div className="mt-6 flex flex-col-reverse gap-3 sm:flex-row sm:justify-end"><button type="button" disabled={deleting} onClick={closeDelete} className="rounded-xl border border-slate-200 px-5 py-3 text-sm font-bold text-slate-700 hover:bg-slate-50 disabled:opacity-50">إلغاء</button><button type="button" disabled={deleting || confirmation !== deleteTarget.name} onClick={() => void deleteCompany()} className="rounded-xl bg-red-600 px-5 py-3 text-sm font-bold text-white hover:bg-red-700 disabled:cursor-not-allowed disabled:opacity-40">{deleting ? "جارٍ الحذف…" : "حذف الشركة نهائيًا"}</button></div>
          </div>
        </div>
      )}
    </main>
...
function buildCompanyTree(companies: Company[]): CompanyTreeNodeData[] {
  ...
  const sortNodes = (items: CompanyTreeNodeData[]) => { items.sort((a, b) => a.company.name.localeCompare(b.company.name, "ar")); for (const item of items) sortNodes(item.children); };
  sortNodes(roots);
  return roots;
}

function CompanyTreeNode({ node, depth, expanded, onToggle, onEnter, onAddSubsidiary, onDelete }: { node: CompanyTreeNodeData; depth: number; expanded: Set<string>; onToggle: (id: string) => void; onEnter: (company: Company) => void; onAddSubsidiary: (company: Company) => void; onDelete: (company: Company) => void }) {
  ...
  return (
    <div className={depth > 0 ? "mr-5 border-r-2 border-slate-200 pr-5 sm:mr-8 sm:pr-7" : ""}>
      <article className="rounded-2xl border border-slate-200 bg-slate-50/60 p-4 transition hover:border-slate-300 hover:bg-white hover:shadow-sm sm:p-5">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex min-w-0 items-center gap-3">
            {hasChildren ? <button type="button" onClick={() => onToggle(company.id)} aria-label={isExpanded ? `طي ${company.name}` : `فتح ${company.name}`} className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl border border-slate-200 bg-white text-slate-600 hover:bg-slate-100">{isExpanded ? "−" : "+"}</button> : <span className="h-9 w-9 shrink-0" />}
            <button type="button" onClick={() => onEnter(company)} className="flex min-w-0 items-center gap-4 text-right">
              ...
              <div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><h4 className="truncate text-base font-bold text-slate-950 sm:text-lg">{company.name}</h4><span className={`rounded-full px-2.5 py-1 text-[11px] font-bold ${depth === 0 ? "bg-slate-200 text-slate-700" : "bg-blue-100 text-blue-700"}`}>{depth === 0 ? "شركة أم" : "شركة تابعة"}</span></div><p className="mt-1 text-xs text-slate-500">{company.industry || "قطاع غير محدد"}{company.city ? ` · ${company.city}` : ""}</p></div>
            </button>
          </div>
          <div className="flex flex-wrap items-center gap-2 sm:justify-end">
            ...
            {hasChildren && <span className="rounded-full bg-blue-50 px-3 py-1.5 text-xs font-semibold text-blue-700">{node.children.length} تابع</span>}
            {company.can_manage_children && <button type="button" onClick={() => onAddSubsidiary(company)} className="rounded-xl border border-slate-200 bg-white px-3.5 py-2 text-xs font-bold text-slate-700 hover:bg-slate-100">+ شركة تابعة</button>}
      ...
      {hasChildren && isExpanded && <div className="mt-3 space-y-3">{node.children.map((child) => <CompanyTreeNode key={child.company.id} node={child} depth={depth + 1} expanded={expanded} onToggle={onToggle} onEnter={onEnter} onAddSubsidiary={onAddSubsidiary} onDelete={onDelete} />)}</div>}
    </div>
  );
}
