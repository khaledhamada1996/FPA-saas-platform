  ...
  useEffect(() => {
    ...
    async function loadCompanies() {
      ...
      const { data: organizations, error: organizationsError } = await supabase
        .from("organizations")
        .select("id, name, legal_name, industry, city, base_currency")
        .in("id", ids)
      ...
      const nextCompanies = (organizations ?? []).map((organization) => ({
        ...organization,
        role: membershipByOrganization.get(organization.id) ?? "viewer",
      }));
    ...
    <main className="min-h-screen bg-[#f7f8fa] px-4 py-6 text-[#172033] sm:px-6 lg:px-10" dir="rtl">
      <header className="mx-auto flex w-full max-w-7xl items-center justify-between gap-4 border-b border-slate-200 pb-5">
        <div className="min-w-0">
          <p className="text-xs font-bold tracking-[0.16em] text-slate-400">FP&A</p>
          <h1 className="mt-1 truncate text-lg font-bold text-slate-950">شركاتي</h1>
        </div>
      ...
      <section className="mx-auto w-full max-w-7xl py-8 sm:py-10 lg:py-14">
        <div className="max-w-3xl">
          <p className="text-xs font-bold tracking-[0.14em] text-slate-400">YOUR WORKSPACES</p>
          <h2 className="mt-3 text-3xl font-bold tracking-tight text-slate-950 sm:text-4xl">اختر الشركة التي تريد العمل عليها</h2>
          <p className="mt-3 text-sm leading-7 text-slate-500 sm:text-base">تظهر لك فقط الشركات التي لديك صلاحية الوصول إليها. عند اختيار شركة نفتح مساحة العمل مباشرة حسب صلاحياتك.</p>
        </div>

        <div className="mt-8 grid gap-5 sm:grid-cols-2 xl:grid-cols-3">
          {companies.map((company) => (
            <button key={company.id} type="button" onClick={() => enterCompany(company)} className="group text-right rounded-3xl border border-slate-200 bg-white p-6 shadow-sm transition hover:-translate-y-0.5 hover:border-slate-400 hover:shadow-md">
              <div className="flex items-start justify-between gap-4">
                <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl bg-slate-950 text-lg font-bold text-white">{company.name.slice(0, 1)}</div>
                <span className="rounded-full bg-slate-100 px-3 py-1.5 text-xs font-semibold text-slate-600">{roleLabels[company.role] ?? company.role}</span>
              </div>
              ...
              <div className="mt-6 flex items-center justify-between border-t border-slate-100 pt-4 text-xs text-slate-400">
                ...
                <span className="font-bold text-slate-700 group-hover:text-slate-950">دخول إلى الشركة ←</span>
              </div>
            </button>
          ))}
        </div>

        <div className="mt-8 rounded-2xl border border-slate-200 bg-white p-5 text-sm leading-7 text-slate-500 sm:p-6">
          <span className="font-bold text-slate-900">فكرة المساحة:</span> نفس الشركة يمكن أن يعمل عليها فريق كامل، لكن كل مستخدم يرى وينفذ فقط ما تسمح به صلاحياته. سنبني إدارة الفريق والصلاحيات كجزء أساسي من مساحة العمل، وليس كميزة ثانوية.
        </div>
      </section>
    </main>
  );
...
function CenteredState({ text }: { text: string }) {
  return (
    <main className="min-h-screen bg-[#f7f8fa] px-4 py-8 text-[#172033]" dir="rtl">
      <div className="mx-auto flex min-h-[calc(100dvh-4rem)] w-full max-w-3xl items-center justify-center">
