    ...
    <main className="min-h-screen bg-[#f7f8fa] text-[#172033]">
      <header className="border-b border-slate-200 bg-white">
        <div className="mx-auto flex max-w-6xl items-center justify-between px-6 py-5 lg:px-10">
          ...
          <div className="text-right"><div className="font-bold text-slate-950">FP&A</div><div className="text-xs text-slate-400">بدء بيئة العمل</div></div>
        </div>
      </header>
        ...
        {!started ? (
          <div className="grid gap-12 lg:grid-cols-[0.8fr_1.2fr] lg:items-start">
            <div>
              ...
              <div className="mt-8 space-y-4 text-sm text-slate-600"><p>✓ لا نخلط بين البيانات الفعلية والموازنة والتوقعات</p><p>✓ لا نعتمد رقمًا ماليًا قبل التحقق من مصدره</p><p>✓ الحسابات المالية الأساسية تُنفذ بطريقة حتمية داخل النظام</p></div>
            </div>
            ...
            <div className="rounded-3xl border border-slate-200 bg-white p-7 shadow-xl shadow-slate-200/40 sm:p-9">
              ...
              <input id="company" value={company} onChange={(event) => setCompany(event.target.value)} placeholder="مثال: شركة القائد" className="mt-3 w-full rounded-xl border border-slate-200 bg-slate-50 px-4 py-4 outline-none transition focus:border-slate-500 focus:bg-white" />
              <div className="mt-8"><p className="text-sm font-bold text-slate-900">كيف ستبدأ بإضافة البيانات؟</p><div className="mt-4 grid gap-3">
              ...
              <button type="button" onClick={handleStart} disabled={!company.trim()} className="mt-8 w-full rounded-xl bg-slate-950 px-6 py-4 font-bold text-white transition hover:bg-slate-800 disabled:cursor-not-allowed disabled:bg-slate-300">إنشاء بيئة العمل والبدء</button>
          ...
          <div className="mx-auto max-w-3xl rounded-3xl border border-slate-200 bg-white p-8 text-center shadow-xl shadow-slate-200/40 sm:p-12">
            ...
            <div className="mt-9 grid gap-3 text-right sm:grid-cols-3">{['إعداد الهيكل المالي', 'إضافة الحسابات', 'إدخال البيانات والتحقق'].map((step, index) => <div key={step} className="rounded-2xl bg-slate-50 p-5"><span className="text-xs font-bold text-slate-400">0{index + 1}</span><p className="mt-5 text-sm font-bold text-slate-900">{step}</p></div>)}</div>
