...
type FilterState = DynamicReportFilterState;
...
const labels: Record<keyof FilterState, string> = { branch: "الفرع", department: "القسم", costCenter: "مركز التكلفة", region: "المنطقة", product: "المنتج", project: "المشروع", account: "الحساب" };
...
export default function TrialBalancePage() {
  ...
  const { options: dynamicOptions, loading: optionsLoading, error: optionsError } = useDynamicReportFilterOptions(org, filters);
  ...
  useEffect(() => {
    ...
    void supabase.from("financial_periods").select("id,period_start,period_end,status").eq("organization_id", id).order("period_start", { ascending: false }).then(({ data: rows, error: e }) => {
      if (e) setError(e.message);
      else { const next = (rows || []) as Period[]; setPeriods(next); if (next[0]) setPeriodId(next[0].id); }
      setLoading(false);
    });
  }, []);
  ...
  async function load() {
    if (!org || !periodId) return;
    setRunning(true); setError("");
    const { data: result, error: e } = await supabase.rpc("get_trial_balance", {
      p_organization_id: org, p_period_id: periodId,
  ...
  useEffect(() => { if (org && periodId) void load(); }, [org, periodId, filters.branch, filters.department, filters.costCenter, filters.region, filters.product, filters.project, filters.account]);
  ...
  const fields: [keyof FilterState, string, Option[]][] = [["branch", "الفرع", options.branches], ["department", "القسم", options.departments], ["costCenter", "مركز التكلفة", options.cost_centers], ["region", "المنطقة", options.regions], ["product", "المنتج", options.products], ["project", "المشروع", options.projects]];
  ...
  return <main dir="rtl" className="min-h-screen bg-[#f7f8fa] text-[#172033]">
    ...
    <section className="px-4 py-6 sm:px-6 lg:px-8">
      <div className="border-b border-slate-200 pb-6"><p className="text-xs font-bold text-slate-400">Trial Balance</p><h2 className="mt-1 text-2xl font-bold text-slate-950 sm:text-3xl">ميزان المراجعة الفعلي</h2><p className="mt-2 text-sm leading-6 text-slate-500">يعتمد التقرير على فترة مالية موجودة في النظام، ثم يطبق الفلاتر على الحقائق الفعلية المنشورة فقط.</p></div>
      <div ref={filterRef} className="mt-5 border border-slate-200 bg-white p-4 shadow-sm">
        <div className="flex flex-wrap items-center gap-2"><label className="text-[11px] font-bold text-slate-500">الفترة المالية<select value={periodId} onChange={e => { setPeriodId(e.target.value); setData(null); }} className="mt-1 min-h-9 border border-slate-300 bg-white px-3 text-xs"><option value="">اختر الفترة</option>{periods.map(p => <option key={p.id} value={p.id}>{formatDate(p.period_start)} — {formatDate(p.period_end)} · {p.status}</option>)}</select></label><button type="button" onClick={() => setOpenFilters(v => !v)} className={`mt-4 min-h-9 border px-3 text-xs font-bold ${activeFilterCount ? "border-slate-950 bg-slate-950 text-white" : "border-slate-300 bg-white"}`}>الأبعاد{activeFilterCount ? ` · ${activeFilterCount}` : ""}⌄</button><button type="button" disabled={!periodId || running} onClick={() => void load()} className="mt-4 min-h-9 bg-slate-950 px-4 text-xs font-bold text-white disabled:opacity-50">{running ? "جاري..." : "تطبيق"}</button></div>
        {activeFilterCount > 0 && <div className="mt-3 flex flex-wrap gap-2">{Object.entries(filters).filter(([, v]) => v).map(([k]) => <button key={k} type="button" onClick={() => setFilter(k as keyof FilterState, "")} className="bg-slate-100 px-3 py-1.5 text-xs font-semibold">{labels[k as keyof FilterState]} ×</button>)}<button type="button" onClick={clear} className="text-xs font-bold text-red-600">مسح الكل</button></div>}
