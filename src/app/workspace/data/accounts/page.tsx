  ...
  return (
    <main dir="rtl" className="min-h-screen bg-[#f7f8fa] text-slate-900">
      ...
      <section className="mx-auto max-w-[1500px] px-4 py-7 sm:px-6 lg:px-8">
        <div className="flex flex-col gap-5 border-b border-slate-200 pb-7 lg:flex-row lg:items-end lg:justify-between"><div><p className="text-xs font-bold text-slate-400">الأساس المحاسبي للنموذج المالي</p><h2 className="mt-2 text-3xl font-bold tracking-tight text-slate-950">دليل حسابات يصل إلى 6 مستويات</h2><p className="mt-3 max-w-3xl text-sm leading-7 text-slate-500">أنشئ دليل الحسابات يدويًا أو استورده من Excel. الحسابات هنا هي المرجع الذي سيظهر لاحقًا كـ«الحساب المستهدف» في Mapping لجميع مصادر البيانات.</p></div><div className="flex flex-wrap gap-2"><a href="/workspace/data/chart-of-accounts" className="rounded-xl border border-slate-300 bg-white px-4 py-3 text-sm font-bold hover:bg-slate-50">استيراد Excel</a><a href="/workspace/data-monitoring/connectors" className="rounded-xl border border-slate-300 bg-white px-4 py-3 text-sm font-bold hover:bg-slate-50">مصادر وربط الأنظمة</a></div></div>
          ...
          <section className="order-2 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm lg:order-1">
            ...
            {loading ? <div className="p-8 text-sm text-slate-500">جارٍ تحميل دليل الحسابات…</div> : filtered.length ? (
              <div className="relative pr-5 sm:pr-7">
                ...
                <div className="relative z-10 flex items-center gap-3 border-b border-slate-200 bg-white px-4 py-2 sm:px-5">
                  <div className="flex shrink-0 items-center gap-1 bg-white"><button type="button" onClick={increaseLevel} disabled={visibleLevel >= 6} aria-label="فتح المستوى التالي بالكامل" title="فتح مستوى كامل" className="flex h-7 w-7 items-center justify-center rounded border border-slate-400 bg-white text-base font-bold text-slate-900 hover:bg-slate-50 disabled:cursor-not-allowed disabled:opacity-40">+</button><button type="button" onClick={decreaseLevel} disabled={visibleLevel <= 1} aria-label="إغلاق المستوى الحالي بالكامل" title="إغلاق مستوى كامل" className="flex h-7 w-7 items-center justify-center rounded border border-slate-400 bg-white text-base font-bold text-slate-900 hover:bg-slate-50 disabled:cursor-not-allowed disabled:opacity-40">−</button></div>
                  <span className="text-[11px] font-bold text-slate-500">المستوى الظاهر: {visibleLevel} من 6</span>
                </div>
                <div className="relative">{renderTree(null, true)}</div>
              </div>
            ) : <div className="p-10 text-center text-sm text-slate-500">لا توجد حسابات بعد. أنشئ أول حساب أو استورد دليل الحسابات من Excel.</div>}
          </section>
          <aside className="order-1 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm lg:order-2"><div className="flex items-center justify-between"><div><p className="text-xs font-bold text-slate-400">{selected ? "تعديل حساب" : "حساب جديد"}</p><h3 className="mt-1 font-bold">إدارة الحساب</h3></div>{selected && <button type="button" onClick={resetForm} className="text-xs font-bold text-slate-500">حساب جديد</button>}</div><div className="mt-5 space-y-3">
