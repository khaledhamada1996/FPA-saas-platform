...
export default function ImportReviewPage() {
  ...
  async function load() {
    ...
    try {
      ...
      const {
        data: result,
        error: reviewError,
      } = await supabase.rpc("get_import_review", { p_import_id: importId });
      ...
      for (const source of next.sources) {
        if (source.target_account_id) {
          initial[source.source_code] = source.target_account_id;
        }
      }
      setTargets(initial);
  ...
  async function callRpc(fn: string, args: Record<string, unknown>) {
    ...
    try {
      ...
      setError(
        rpcError instanceof Error ? rpcError.message : "تعذر تنفيذ العملية",
      );
    } finally {
      setSaving(false);
    }
  ...
  if (loading) {
    return (
      <main className="min-h-screen bg-[#f7f8fa] p-6 text-[#172033]" dir="rtl">
        <div className="mx-auto max-w-6xl rounded-2xl border border-slate-200 bg-white p-10 text-center">
          جاري تحميل مراجعة الاستيراد…
        </div>
      </main>
    );
  }
    ...
    return (
      <main className="min-h-screen bg-[#f7f8fa] p-6 text-[#172033]" dir="rtl">
        <div className="mx-auto max-w-3xl rounded-2xl border border-red-200 bg-red-50 p-8 text-center text-red-800">
          {error}
          <button
            onClick={() => router.push("/workspace/data")}
            className="mt-5 block w-full rounded-xl bg-slate-950 px-5 py-3 text-white"
          >
            العودة للاستيراد
          </button>
        </div>
  ...
  const ready = imp.status === "ready_for_review";
      ...
      <main className="min-h-screen bg-[#f7f8fa] p-6 text-[#172033]" dir="rtl">
        <div className="mx-auto max-w-3xl rounded-2xl border border-amber-200 bg-amber-50 p-8 text-center">
          ...
          <p className="mt-3 text-sm leading-7 text-slate-600">
            الوصول إلى الاستيراد والمطابقة يتم على مستوى المستخدم والصلاحية، وليس بمجرد امتلاك دور عام.
          </p>
          <button
            onClick={() => router.push("/workspace/data")}
      ...
      <header className="border-b border-slate-200 bg-white">
        <div className="mx-auto flex max-w-[1500px] items-center justify-between gap-4 px-4 py-4 sm:px-6 lg:px-8">
          ...
          <div className="text-right">
            <p className="text-[10px] font-bold tracking-[0.14em] text-slate-400">IMPORT REVIEW</p>
          ...
          <div>
            ...
            <div className="mt-2 flex flex-wrap items-center gap-3">
              ...
              <span className="rounded-full bg-slate-100 px-3 py-1 text-xs font-bold text-slate-600">
                {imp.row_count.toLocaleString("ar-SA")} صف
              </span>
            </div>
            <p className="mt-3 max-w-3xl text-sm leading-7 text-slate-500">
              المسار الإجباري: مطابقة الحسابات ثم تجهيز المراجعة ثم المطابقة النهائية ثم النشر. لا تصبح البيانات Actuals قبل النشر.
            </p>
          </div>
          <div className="flex flex-wrap gap-2 text-xs font-bold">
            <span className={`rounded-full px-3 py-2 ${imp.status !== "mapping_required" ? "bg-emerald-50 text-emerald-700" : "bg-amber-50 text-amber-700"}`}>1 المطابقة</span>
            <span className={`rounded-full px-3 py-2 ${ready || published ? "bg-emerald-50 text-emerald-700" : "bg-slate-100 text-slate-500"}`}>2 المراجعة</span>
            <span className={`rounded-full px-3 py-2 ${recon ? "bg-emerald-50 text-emerald-700" : "bg-slate-100 text-slate-500"}`}>3 المطابقة النهائية</span>
            <span className={`rounded-full px-3 py-2 ${published ? "bg-emerald-50 text-emerald-700" : "bg-slate-100 text-slate-500"}`}>4 النشر</span>
          </div>
        </div>
        ...
        <div className="mt-6 grid gap-5 lg:grid-cols-[minmax(0,1fr)_320px]">
          <section className="min-w-0 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:p-7">
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              <div>
                <h3 className="text-lg font-bold text-slate-950">مطابقة الحسابات المصدر</h3>
                <p className="mt-1 text-sm text-slate-500">الإصدار <b>{version}</b> · الحسابات غير المعتمدة لا تسمح بالنشر.</p>
              </div>
            ...
            <div className="mt-5 overflow-x-auto">
              <table className="w-full min-w-[780px] text-right text-sm">
                <thead className="border-b border-slate-200 text-xs text-slate-500">
                  <tr><th className="p-3">الحساب المصدر</th><th className="p-3">عدد الصفوف</th><th className="p-3">الحالة</th><th className="p-3">الحساب المستهدف</th><th className="p-3">إجراء</th></tr>
                </thead>
                  ...
                  {filtered.map((source) => {
                    ...
                    return (
                      <tr key={source.source_code}>
                        <td className="p-3"><b>{source.source_code}</b><div className="mt-1 text-xs text-slate-500">{source.source_name}</div></td>
                        <td className="p-3">{source.row_count.toLocaleString("ar-SA")}</td>
                        <td className="p-3">{source.mapping_status ?? "غير مطابق"}</td>
                        <td className="p-3">
                        ...
                        <td className="p-3"><button disabled={saving || !canSave} onClick={() => saveMapping(source)} className="rounded-lg bg-slate-950 px-4 py-2 text-xs font-bold text-white disabled:cursor-not-allowed disabled:opacity-40">حفظ</button></td>
                      </tr>
                    );
                  })}
                </tbody>
          ...
          <aside className="space-y-5">
            <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
              ...
              <div className="mt-4 space-y-2">
                <button disabled={saving || imp.status !== "mapping_required" || !canPrepare} onClick={prepare} className="w-full rounded-xl bg-slate-950 px-4 py-3 text-sm font-bold text-white disabled:opacity-40">تجهيز للمراجعة</button>
                <button disabled={saving || imp.status !== "ready_for_review" || !canReconcile} onClick={reconcile} className="w-full rounded-xl border border-slate-300 bg-white px-4 py-3 text-sm font-bold text-slate-900 disabled:opacity-40">تنفيذ المطابقة النهائية</button>
                <button disabled={saving || !ready || !recon || recon.status !== "passed" || !canPublish} onClick={publish} className="w-full rounded-xl bg-emerald-700 px-4 py-3 text-sm font-bold text-white disabled:opacity-40">نشر Actuals</button>
              </div>
            ...
            {recon && (
              <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                <h3 className="font-bold text-slate-950">المطابقة</h3>
                ...
                <p className={`mt-4 rounded-lg p-3 text-xs font-bold ${recon.status === "passed" ? "bg-emerald-50 text-emerald-700" : "bg-amber-50 text-amber-700"}`}>الحالة: {recon.status}</p>
              </section>
            )}
            ...
            {published && !rolledBack && canRollback && (
              <section className="rounded-2xl border border-red-200 bg-white p-5 shadow-sm">
                ...
                <p className="mt-2 text-xs leading-6 text-slate-500">التراجع عملية حوكمية وتتطلب صلاحية التراجع وتسجل السبب في سجل التدقيق.</p>
