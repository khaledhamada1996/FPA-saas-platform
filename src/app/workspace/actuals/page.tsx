  ...
  return <main className="min-h-screen bg-[#f7f8fa] text-[#172033]" dir="rtl">
    ...
    <section className="mx-auto w-full px-4 py-7 sm:px-6 sm:py-9 lg:px-8 lg:py-10">
      ...
      {error ? <div className="mt-7 border border-rose-200 bg-rose-50 p-5 text-sm text-rose-900">{error}</div> : <>
        <div className="mt-7 grid gap-3 sm:grid-cols-2 xl:grid-cols-3">{cards.map(([label,value])=><article key={label} className="border border-slate-200 bg-white p-5 shadow-sm sm:p-6"><p className="text-sm text-slate-500">{label}</p><p className="mt-3 text-2xl font-bold text-slate-950 sm:text-3xl">{loading?"…":formatMoneyMinor(value)}</p><p className="mt-2 text-xs text-slate-400">ريال سعودي</p></article>)}</div>
        <section className="mt-5 border border-slate-200 bg-white shadow-sm"><div className="border-b border-slate-200 p-4 sm:p-6"><h3 className="font-bold text-slate-950">حالة البيانات المنشورة</h3><p className="mt-1 text-xs leading-6 text-slate-500 sm:text-sm">المصدر هو Financial Facts بحالة Actual / Published.</p></div><div className="grid gap-4 p-5 sm:grid-cols-3 sm:p-6"><div><p className="text-xs text-slate-400">عدد السجلات المنشورة</p><p className="mt-1 text-lg font-bold text-slate-950">{loading?"…":formatNumber(summary.rows)}</p></div><div><p className="text-xs text-slate-400">الحالة</p><p className="mt-1 text-lg font-bold text-emerald-700">{loading?"…":summary.rows>0?"منشور":"لا توجد بيانات"}</p></div><div><Link href="/workspace/data/import" className="inline-flex min-h-11 items-center justify-center bg-slate-950 px-5 py-3 text-sm font-bold text-white hover:bg-slate-800">إدارة القيود اليومية</Link></div></div></section>
      </>}
    </section>
  </main>;
}
