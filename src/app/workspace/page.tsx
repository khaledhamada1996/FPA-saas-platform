...
const navigation = [
  ...
  ["variance", "الفروقات"], ["cash", "التدفق النقدي"], ["scenarios", "السيناريوهات"],
] as const;
...
export default function WorkspacePage() {
  ...
  useEffect(() => {
    ...
    async function loadWorkspace() {
      ...
      if (!userData.user) { if (activeEffect) setAuthState("unauthenticated"); router.replace("/login?next=/workspace"); return; }
      const { data, error } = await supabase.rpc("get_my_workspaces");
      if (error) { if (activeEffect) router.replace("/start"); return; }
      const list = (data ?? []) as Company[];
      if (!list.length) { router.replace("/start"); return; }
      ...
      setCompany(selected); setCompanies(list); setAuthState("authenticated");
    }
    void loadWorkspace();
    const { data: listener } = supabase.auth.onAuthStateChange((_event, session) => {
      if (!activeEffect) return;
      if (!session?.user) { setAuthState("unauthenticated"); router.replace("/login?next=/workspace"); }
    });
  ...
  function switchCompany(id: string) {
    ...
    if (!next) return;
    window.sessionStorage.setItem("activeOrganizationId", next.id); setCompany(next); setActive("overview");
  }
