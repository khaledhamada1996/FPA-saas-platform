  ...
  <section className="mx-auto max-w-[1500px] px-4 py-6 sm:px-6 lg:px-8 lg:py-8">
   ...
   {loading?<div className="mt-6 border border-slate-200 bg-white p-10 text-center text-sm text-slate-500">جارٍ تحميل التكاملات…</div>:<>
    <div className="mt-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
     {providers.map(c=>{const m=providerMeta[c.connector_key];const s=states.find(x=>x.connector_id===c.id);return <button key={c.id} type="button" onClick={()=>setSelected(c.connector_key)} className={`border p-5 text-right ${selected===c.connector_key?"border-slate-500 bg-slate-50":"border-slate-200 bg-white hover:bg-slate-50"}`}><div className="flex items-start justify-between gap-3"><div><h2 className="font-bold text-[#292929]">{m.title}</h2><p className="mt-2 text-xs leading-5 text-slate-500">{m.description}</p></div><span className="text-[10px] font-bold text-slate-500">{s?.has_credential?"متصل":"غير متصل"}</span></div><div className="mt-4 border-t border-slate-100 pt-3 text-[10px] font-bold text-slate-400">{m.stage}</div></button>})}
    </div>
