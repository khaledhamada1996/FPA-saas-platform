...
export default function IntegrationsPage(){
 ...
 const publish=<div className="flex flex-wrap items-center gap-2">{publishConfirm&&<div className="w-full border border-slate-300 bg-slate-50 p-3 text-xs leading-5 text-slate-700">تم التحقق من الجاهزية. هذه معاينة الدفعة قبل إنشاء القيود الفعلية.{publishPreview&&<div className="mt-3 grid grid-cols-2 gap-2 sm:grid-cols-4"><div><b>{publishPreview.row_count}</b><span className="mr-1">سجل</span></div><div><b>{(publishPreview.debit_minor/100).toLocaleString("ar-SA",{minimumFractionDigits:2})}</b><span className="mr-1">مدين</span></div><div><b>{(publishPreview.credit_minor/100).toLocaleString("ar-SA",{minimumFractionDigits:2})}</b><span className="mr-1">دائن</span></div><div><b>{(publishPreview.net_minor/100).toLocaleString("ar-SA",{minimumFractionDigits:2})}</b><span className="mr-1">صافي</span></div></div>}<div className="mt-2">الفترة: {publishPreview?.date_from||"—"} إلى {publishPreview?.date_to||"—"} · العملات: {publishPreview?.currencies?.join("، ")||"—"}</div><div className="mt-1 font-bold">التأكيد سيكتب البيانات في القوائم المالية ولا يمكن نشر نفس الدفعة مرة أخرى.</div></div>}<button type="button" disabled={!canPublish||busy==="publish"||reconciliation.status!=="passed"} onClick={()=>void publishCurrent()} className="border border-slate-500 bg-slate-800 px-4 py-2.5 text-xs font-bold text-white disabled:cursor-not-allowed disabled:opacity-40">{busy==="publish"?"جارٍ التنفيذ…":publishConfirm?"تأكيد النشر الفعلي":"نشر البيانات الفعلية"}</button>{publishConfirm&&<button type="button" disabled={!!busy} onClick={()=>{setPublishConfirm(false);setPublishPreview(null)}} className="border border-slate-300 bg-white px-4 py-2.5 text-xs font-bold text-slate-600">إلغاء</button>}<span className="text-[11px] text-slate-500">{reconciliation.status==="passed"?"المصدر اجتاز المصالحة. النشر متاح لصاحب الصلاحية.":"النشر يتاح فقط بعد اكتمال المطابقة والمصالحة."}</span></div>
 return <main dir="rtl" className="min-h-screen bg-white text-[#292929]">
  ...
  <section className="mx-auto max-w-[1500px] px-4 py-5 sm:px-6 lg:px-8 lg:py-7">
   ...
   {loading?<div className="mt-6 border border-slate-200 bg-white p-10 text-center text-sm text-slate-500">جارٍ تحميل التكاملات…</div>:<>
    ...
    {currentState&&review&&reviewQueueTotal>0&&(
      <section className="mt-5 border border-slate-200 bg-white p-5 sm:p-6">
       <div className="flex items-center justify-between border-b border-slate-100 pb-4">
        <div>
         <p className="text-[10px] font-bold tracking-[0.14em] text-slate-400">REVIEW QUEUE</p>
         <h2 className="mt-1 text-lg font-bold">سجلات تحتاج مراجعة</h2>
         <p className="mt-1 text-xs text-slate-500">لا يتم نشر أي سجل غير مكتمل.</p>
        </div>
        <span className="border border-slate-300 bg-slate-50 px-3 py-1.5 text-xs font-bold text-slate-600">{reviewQueueTotal} سجل</span>
       </div>
       <div className="mt-4 flex flex-wrap gap-2 border-b border-slate-100 pb-4">
        {[
         ...
         <button key={value} type="button" onClick={()=>setReviewQueueFilter(value as "needs_review"|"rejected"|"unmapped"|"all")} className={`border px-3 py-1.5 text-[11px] font-bold ${reviewQueueFilter===value?"border-slate-500 bg-slate-800 text-white":"border-slate-300 bg-white text-slate-600"}`}>
          {label}
         </button>
        ))}
       </div>
        ...
        {reviewQueue.map(item=>(
         <div key={item.id} className="border border-slate-200 p-3">
          <div className="grid gap-2 sm:grid-cols-5 text-xs">
           <div><span className="text-slate-400">التاريخ</span><div className="font-semibold">{item.transaction_date??"—"}</div></div>
           <div><span className="text-slate-400">المرجع</span><div className="font-semibold">{item.journal_no||item.source_record_key}</div></div>
           <div><span className="text-slate-400">الحساب</span><div className="font-semibold">{item.account_code||"—"} {item.account_name||""}</div></div>
           <div><span className="text-slate-400">مدين</span><div className="font-semibold">{item.debit_minor}</div></div>
           <div><span className="text-slate-400">دائن</span><div className="font-semibold">{item.credit_minor}</div></div>
          </div>
          <div className="mt-3 border-t border-slate-100 pt-3 text-xs">
           <div className="font-semibold text-slate-700">{item.mapping_status!=="mapped"?"مطابقة الحساب تحتاج مراجعة":item.dimension_mapping_status!=="mapped"?"مطابقة الأبعاد تحتاج مراجعة":"يحتاج مراجعة"}</div>
           <div className="mt-1 text-[10px] text-slate-500">{item.mapping_message||item.dimension_mapping_message||item.normalization_message||""}</div>
           {item.mapping_status!=="mapped"&&(
            <div className="mt-2 flex flex-wrap gap-2">
             <select value={mappingTargets[item.id]??""} onChange={e=>setMappingTargets(x=>({...x,[item.id]:e.target.value}))} className="min-w-[210px] border border-slate-300 bg-white px-2 py-1.5 text-[11px]">
              <option value="">اختر الحساب المستهدف</option>
              {accounts.map(a=><option key={a.id} value={a.id}>{a.code?a.code+" — ":""}{a.name}</option>)}
             </select>
             <button disabled={mappingBusy===item.id} onClick={()=>void saveReviewAccount(item)} className="border border-slate-300 bg-slate-100 px-3 py-1.5 text-[11px] font-bold">{mappingBusy===item.id?"جارٍ الحفظ…":"حفظ المطابقة"}</button>
             {item.account_mapping_id&&<button disabled={mappingBusy===item.id} onClick={()=>void approveReviewAccount(item)} className="border border-slate-400 bg-slate-200 px-3 py-1.5 text-[11px] font-bold">{mappingBusy===item.id?"جارٍ التنفيذ…":"اعتماد المطابقة"}</button>}
            </div>
           )}
           {item.dimension_mapping_status!=="mapped"&&Object.entries(item.dimensions??{}).map(([sourceDimension,sourceValue])=>(
            <div key={sourceDimension} className="mt-2 border border-slate-200 bg-slate-50 p-2">
             <div className="mb-1 text-[10px] font-semibold text-slate-600">{sourceDimension}: {String(sourceValue)}</div>
             <div className="flex flex-wrap gap-2">
              <select value={dimensionTypes[item.id+":"+sourceDimension]??""} onChange={e=>setDimensionTypes(x=>({...x,[item.id+":"+sourceDimension]:e.target.value}))} className="min-w-[150px] border border-slate-300 bg-white px-2 py-1.5 text-[11px]">
               <option value="">نوع البعد</option>
               {[["legal_entity","شركة"],["branch","فرع"],["department","إدارة"],["cost_center","مركز تكلفة"],["region","منطقة"],["product","منتج"],["project","مشروع"]].map(([v,l])=><option key={v} value={v}>{l}</option>)}
              </select>
              <select disabled={!dimensionTypes[item.id+":"+sourceDimension]} value={dimensionTargets[item.id+":"+sourceDimension]??""} onChange={e=>setDimensionTargets(x=>({...x,[item.id+":"+sourceDimension]:e.target.value}))} className="min-w-[190px] border border-slate-300 bg-white px-2 py-1.5 text-[11px]">
               <option value="">القيمة المستهدفة</option>
               {(dimensionOptions[dimensionTypes[item.id+":"+sourceDimension]??""]??[]).map(a=><option key={a.id} value={a.id}>{a.code?a.code+" — ":""}{a.name}</option>)}
              </select>
              <button disabled={!!mappingBusy} onClick={()=>void saveReviewDimension(item,sourceDimension)} className="border border-slate-300 bg-white px-3 py-1.5 text-[11px] font-bold">{mappingBusy===item.id+":"+sourceDimension?"جارٍ الحفظ…":"حفظ البعد"}</button>
              <button disabled={!!mappingBusy} onClick={()=>void approveReviewDimension(item,sourceDimension)} className="border border-slate-400 bg-slate-200 px-3 py-1.5 text-[11px] font-bold">اعتماد البعد</button>
             </div>
            </div>
           ))}
          </div>
