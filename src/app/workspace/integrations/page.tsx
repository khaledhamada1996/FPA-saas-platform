 ...
 return <main dir="rtl" className="min-h-screen bg-white text-[#292929]">
  ...
  <section className="mx-auto max-w-[1500px] px-4 py-5 sm:px-6 lg:px-8 lg:py-7">
   ...
   {loading?<div className="mt-6 border border-slate-200 bg-white p-10 text-center text-sm text-slate-500">جارٍ تحميل التكاملات…</div>:<>
    ...
    {currentState&&review&&reviewQueueTotal>0&&<section className="mt-5 border border-slate-200 bg-white p-5 sm:p-6"><div className="flex items-center justify-between border-b border-slate-100 pb-4"><div><p className="text-[10px] font-bold tracking-[0.14em] text-slate-400">REVIEW QUEUE</p><h2 className="mt-1 text-lg font-bold">سجلات تحتاج مراجعة</h2><p className="mt-1 text-xs text-slate-500">لا يتم نشر أي سجل غير مكتمل. هذه القائمة للعرض والمراجعة قبل المصالحة.</p></div><span className="border border-slate-300 bg-slate-50 px-3 py-1.5 text-xs font-bold text-slate-600">{reviewQueueTotal} سجل</span></div><div className="mt-4 overflow-x-auto"><table className="min-w-full text-right text-xs"><thead><tr className="border-b border-slate-200 text-slate-500"><th className="px-3 py-2 font-semibold">التاريخ</th><th className="px-3 py-2 font-semibold">المرجع</th><th className="px-3 py-2 font-semibold">الحساب المصدر</th><th className="px-3 py-2 font-semibold">مدين</th><th className="px-3 py-2 font-semibold">دائن</th><th className="px-3 py-2 font-semibold">الحالة</th></tr></thead><tbody>{reviewQueue.map(item=><tr key={item.id} className="border-b border-slate-100"><td className="whitespace-nowrap px-3 py-2">{item.transaction_date??"—"}</td><td className="px-3 py-2">{item.journal_no||item.source_record_key}</td><td className="max-w-[280px] px-3 py-2"><div className="font-semibold">{item.account_code||"—"} {item.account_name||""}</div><div className="mt-0.5 text-[10px] text-slate-400">{item.account_external_id||item.description||"—"}</div></td><td className="px-3 py-2 tabular-nums">{item.debit_minor}</td><td className="px-3 py-2 tabular-nums">{item.credit_minor}</td><td className="min-w-[320px] px-3 py-2"><span className="font-semibold text-slate-700">{item.mapping_status!=="mapped"?"مطابقة الحساب تحتاج مراجعة":item.dimension_mapping_status!=="mapped"?"مطابقة الأبعاد تحتاج مراجعة":"يحتاج مراجعة"}</span><div className="mt-1 text-[10px] leading-4 text-slate-500">{item.mapping_message||item.dimension_mapping_message||item.normalization_message||""}</div>{item.mapping_status!=="mapped"&&<div className="mt-2 flex gap-2"><select value={mappingTargets[item.id]??""} onChange={e=>setMappingTargets(x=>({...x,[item.id]:e.target.value}))} className="min-w-[210px] border border-slate-300 bg-white px-2 py-1.5 text-[11px]"><option value="">اختر الحساب المستهدف</option>{accounts.map(a=><option key={a.id} value={a.id}>{a.code?a.code+" — ":""}{a.name}</option>)}</select><button disabled={mappingBusy===item.id} onClick={()=>void saveReviewAccount(item)} className="border border-slate-300 bg-slate-100 px-3 py-1.5 text-[11px] font-bold">{mappingBusy===item.id?"جارٍ الحفظ…":"حفظ المطابقة"}</button></div>}</td><td className="px-3 py-2"><span className="font-semibold text-slate-700">{item.mapping_status!=="mapped"?"مطابقة الحساب تحتاج مراجعة":item.dimension_mapping_status!=="mapped"?"مطابقة الأبعاد تحتاج مراجعة":"يحتاج مراجعة"}</span><div className="mt-1 text-[10px] leading-4 text-slate-500">{item.mapping_message||item.dimension_mapping_message||item.normalization_message||""}</div></td></tr>)}</tbody></table></div></section>}
