...
type RoutePermission = readonly [string, string, string];

// Keep the most-specific routes before their parent prefixes. This prevents
// compatibility aliases from accidentally inheriting a broader permission.
const routePermissions: RoutePermission[] = [
  ...
  ["/workspace/data/chart-of-accounts", "screen.accounts.view", "دليل الحسابات"],
...
function AccessDenied({ name }: { name: string }) {
  ...
  return <section className="mx-auto flex min-h-[calc(100vh-4rem)] w-full max-w-2xl items-center justify-center px-4 py-10 sm:px-6" dir="rtl">
    <div className="w-full border border-slate-200 bg-white p-7 text-center shadow-sm sm:p-10">
      <p className="text-xs font-bold tracking-[0.16em] text-slate-400">FP&A ACCESS CONTROL</p>
      <h1 className="mt-4 text-2xl font-bold text-slate-950 sm:text-3xl">ليس لديك صلاحية الدخول إلى {name}</h1>
      <p className="mx-auto mt-4 max-w-lg text-sm leading-7 text-slate-500 sm:text-base">ليس لديك الصلاحية المطلوبة لاستخدام هذه الشاشة. تواصل مع مدير النظام لفتح الصلاحية لك.</p>
      <div className="mt-7 flex flex-col justify-center gap-3 sm:flex-row">
        <button type="button" onClick={() => router.push("/workspace")} className="bg-slate-950 px-6 py-3.5 text-sm font-bold text-white hover:bg-slate-800">العودة إلى مساحة العمل</button>
        <button type="button" onClick={() => router.push("/start")} className="border border-slate-300 bg-white px-6 py-3.5 text-sm font-bold text-slate-700 hover:bg-slate-50">شركاتي</button>
      </div>
    </div>
  </section>;
}
...
function WorkspaceContextBar() {
  ...
  function switchCompany(company: Company) {
    ...
    window.sessionStorage.setItem("selectedOrganizationIds", JSON.stringify([company.id]));
