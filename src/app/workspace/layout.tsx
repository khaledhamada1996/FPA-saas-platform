...
const routePermissions: Array<[string, string]> = [
  ...
  ["/workspace/data-monitoring/connectors", "screen.connector_management.view"], ["/workspace/integrations", "screen.connector_management.view"], ["/workspace/data-monitoring", "screen.data_monitoring.view"],
...
export default function WorkspaceLayout({ children }: Readonly<{ children: React.ReactNode }>) {
  ...
  useEffect(() => {
    ...
    const supabase = getSupabaseBrowserClient(); let active = true;
    async function authorize() {
      ...
      if (!allowed) { if (active) setStatus("denied"); router.replace(`/workspace/access-denied?permission=${encodeURIComponent(requiredPermission)}`); return; }
