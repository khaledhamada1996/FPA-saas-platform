...
type DataState = { loading: boolean; authenticated: boolean; hasActuals: boolean; batchCount: number; factCount: number; latestPublishedAt: string | null; error: string };
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    async function loadDataState() {
      ...
      let selectedId = "";
      try { selectedId = window.localStorage.getItem("fpa_workspace_id") ?? ""; } catch { /* ignore */ }

      let membershipQuery = supabase.from("organization_members").select("organization_id, organizations(id, name)").eq("user_id", user.id);
      if (selectedId) membershipQuery = membershipQuery.eq("organization_id", selectedId);
      ...
      setWorkspace(organization ? { id: organization.id, name: organization.name } : { id: organizationId, name: "مساحة العمل" });
  ...
  return (
    <div className="app-shell" dir="rtl">
      <aside className="sidebar">
        ...
        <a className="workspace-switcher" href="/workspace"><span className="workspace-label">مساحة العمل</span><strong>{workspace?.name ?? "مساحة العمل"}</strong><span className="workspace-meta">{dataState.hasActuals ? `${dataState.factCount.toLocaleString("ar-SA")} حقيقة مالية منشورة` : "لا توجد بيانات مالية منشورة بعد"}</span><span aria-hidden="true">تغيير مساحة العمل ←</span></a>
      ...
      <main className="dashboard-main">
        <header className="topbar"><div><p className="breadcrumb">{workspace?.name ?? "مساحة العمل"} / <strong>نظرة عامة</strong></p><h1>نظرة عامة</h1></div><div className="topbar-actions"><span className="data-state-badge">{statusText}</span><span className="top-user" aria-hidden="true">خ</span></div></header>

        <div className="content" id="overview">
          <section className="data-state-banner" role="status"><div className="state-icon">{dataState.hasActuals ? "✓" : "!"}</div><div><strong>{dataState.loading ? "جارٍ قراءة حالة البيانات" : dataState.hasActuals ? "البيانات الفعلية متاحة للنموذج المالي" : "لا توجد مؤشرات مالية معروضة حاليًا"}</strong><p>{dataState.loading ? "يتم التحقق من مساحة العمل والدفعات المنشورة من قاعدة البيانات." : dataState.hasActuals ? `تم نشر ${dataState.batchCount.toLocaleString("ar-SA")} دفعة فعلية تحتوي على ${dataState.factCount.toLocaleString("ar-SA")} حقيقة مالية. المؤشرات ستُحسب فقط بعد اكتمال تعريف الحسابات وقواعد القوائم والتخطيط.` : dataState.authenticated ? "لا يتم عرض أي رقم أو نسبة أو رسم مالي قبل وجود بيانات فعلية تم استيرادها والتحقق منها ونشرها داخل النموذج المالي." : "هذه الواجهة مؤقتة أثناء تأجيل تسجيل الدخول. البيانات المالية لا تُعرض من المتصفح قبل وجود جلسة مستخدم ومساحة عمل مصرح بها."}</p>{dataState.error && <small>{dataState.error}</small>}</div></section>
          ...
          <section className="bottom-grid"><article className="panel empty-panel" id="cash"><div className="panel-header"><div><h3>توقع السيولة</h3><p>الرصيد الافتتاحي والتحصيلات والمدفوعات والتمويل والنفقات الرأسمالية</p></div><span className="section-state">غير متاح بعد</span></div><div className="cash-empty"><strong>لا يوجد توقع نقدي محسوب</strong><p>سيُحسب وفق المعادلة المعتمدة: الرصيد الافتتاحي + التحصيلات المتوقعة − المدفوعات المتوقعة + التمويل − النفقات الرأسمالية.</p></div></article><article className="panel empty-panel"><div className="panel-header"><div><h3>حالة التخطيط</h3><p>دورة الميزانية والتوقع والسيناريوهات</p></div></div><div className="planning-list"><div><span>البيانات الفعلية</span><b>{dataState.hasActuals ? "منشورة" : "غير منشورة"}</b></div><div><span>الميزانية</span><b>لم تُنشأ</b></div><div><span>التوقع</span><b>لم يُنشأ</b></div><div><span>السيناريوهات</span><b>لم تُنشأ</b></div></div></article></section>
