create or replace function public.publish_actuals_from_import(p_import_id uuid,p_mapping_version text) returns uuid language plpgsql security definer set search_path to 'public','pg_catalog' as $function$
declare v_user uuid:=auth.uid(); v_org uuid; v_batch uuid; v_rows integer; v_bad integer; v_status text; v_currency text; v_unresolved integer;
begin
...
if v_status in ('rolled_back','published') then raise exception 'Import cannot be published in its current status'; end if;
...
if exists(select 1 from public.import_rows ir where ir.import_id=p_import_id and not exists(select 1 from public.account_mappings m where m.organization_id=v_org and m.mapping_version=trim(p_mapping_version) and m.source_code=(ir.payload->>'account_code') and m.status='approved')) then raise exception 'Every account must have an approved mapping'; end if;
with vals as (
...
union all select 'project',nullif(btrim(payload->>'project'),'') from public.import_rows where import_id=p_import_id and validation_status='valid'),needed as (select distinct dimension_type,source_value from vals where source_value is not null)
select count(*) into v_unresolved from needed n where not exists(select 1 from public.import_dimension_mappings dm where dm.import_id=p_import_id and dm.organization_id=v_org and dm.dimension_type=n.dimension_type and lower(btrim(dm.source_value))=lower(btrim(n.source_value)) and dm.status='approved');
...
insert into public.financial_periods(organization_id,period_start,period_end,status) select v_org,m.period_start,(m.period_start+interval '1 month - 1 day')::date,'open' from (select distinct date_trunc('month',(payload->>'date')::date)::date period_start from public.import_rows where import_id=p_import_id and validation_status='valid') m on conflict(organization_id,period_start) do nothing;
...
select ir.organization_id,fp.id,am.target_account_id,le.target_id,br.target_id,dep.target_id,cc.target_id,rg.target_id,pr.target_id,pj.target_id,v_currency,round(coalesce(nullif(ir.payload->>'debit','')::numeric,0)*100)::bigint-round(coalesce(nullif(ir.payload->>'credit','')::numeric,0)*100)::bigint,'actual',p_import_id,ir.source_key,ir.payload->>'journal_no',ir.payload->>'description',round(coalesce(nullif(ir.payload->>'debit','')::numeric,0)*100)::bigint,round(coalesce(nullif(ir.payload->>'credit','')::numeric,0)*100)::bigint,(ir.payload->>'date')::date,'published'
from public.import_rows ir join public.account_mappings am on am.organization_id=v_org and am.mapping_version=trim(p_mapping_version) and am.source_code=(ir.payload->>'account_code') and am.status='approved' join public.financial_periods fp on fp.organization_id=v_org and fp.period_start=date_trunc('month',(ir.payload->>'date')::date)::date
...
insert into public.actuals_publish_batches(organization_id,import_id,mapping_version,status,row_count,published_at,published_by) values(v_org,p_import_id,trim(p_mapping_version),'published',v_rows,now(),v_user) returning id into v_batch;
...
insert into public.import_audit_events(organization_id,import_id,actor_id,action,from_status,to_status,metadata) values(v_org,p_import_id,v_user,'publish',v_status,'published',jsonb_build_object('row_count',v_rows,'mapping_version',trim(p_mapping_version),'dimensions_resolved',true,'dimension_mapping_source','approved_import_dimension_mappings'));
return v_batch;
end;$function$;