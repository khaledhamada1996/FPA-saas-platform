-- Secure external integration credentials in Supabase Vault.
create or replace function public.save_connector_secret(
...
begin
  ...
  where dsc.organization_id=p_organization_id and dsc.connector_id=p_connector_id and dsc.enabled=true
  limit 1;
  ...
  if v_existing is null then
    select vault.create_secret(p_secret,v_name,'FP&A external integration credential for ' || v_provider) into v_secret_id;
  else
    perform vault.update_secret(v_existing,p_secret,v_name,'FP&A external integration credential for ' || v_provider);
    v_secret_id := v_existing;
  end if;

  update public.data_source_connectors
  set sync_config=jsonb_set(coalesce(sync_config,'{}'::jsonb),'{credential_ref}',to_jsonb(v_name),true),updated_at=now()
  where id=v_binding and organization_id=p_organization_id;

  perform public.write_audit_event(p_organization_id,'connector.secret_updated','data_source_connector',v_binding::text,null,jsonb_build_object('connector_key',v_provider,'credential_ref',v_name));
  return v_secret_id;
end;
$$;
...
begin
  ...
  from public.data_source_connectors dsc
  join public.connector_catalog c on c.id=dsc.connector_id
  join public.data_sources ds on ds.id=dsc.data_source_id
