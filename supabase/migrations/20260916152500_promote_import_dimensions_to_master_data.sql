...
begin
  ...
  if not exists (select 1 from public.organization_members om where om.organization_id=v_org and om.user_id=v_uid and om.status='active') then raise exception 'ORG_ACCESS_DENIED'; end if;
  ...
  foreach v_type in array v_types loop
    ...
    for v_value in execute format($q$
      select distinct nullif(trim(payload->>%L),'') from public.import_rows where import_id=$1 and nullif(trim(payload->>%L),'') is not null order by 1
    $q$,v_type,v_type) using p_import_id loop
      v_id := null;
      v_code := upper(left(md5(v_org::text||':'||v_type||':'||lower(v_value)),12));
      if v_type='legal_entity' then
        ...
        if v_id is null then insert into public.legal_entities(organization_id,name,code,currency) values(v_org,v_value,v_code,'SAR') returning id into v_id; v_created:=v_created+1; else v_reused:=v_reused+1; end if;
        v_legal_entity_id := v_id;
      ...
      insert into public.import_dimension_mappings(organization_id,import_id,dimension_type,source_value,target_id,created_by,status)
      values(v_org,p_import_id,v_type,v_value,v_id,v_uid,'draft')
      on conflict (import_id,dimension_type,lower(source_value)) do update set target_id=excluded.target_id,updated_at=now(),status='draft';
  ...
  values(v_org,v_uid,'master_data.promoted_from_import','import',p_import_id,jsonb_build_object('created',v_created,'reused',v_reused,'dimension_types',v_types));
