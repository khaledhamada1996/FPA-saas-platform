...
declare
  ...
  v_opening_date date;
  v_row_count integer;
  v_total_debit bigint; v_total_credit bigint;
  v_total_opening_debit bigint; v_total_opening_credit bigint;
  v_total_closing_debit bigint; v_total_closing_credit bigint;
  v_rows jsonb;
begin
  ...
  opening_baseline as (
    ...
    where ob.organization_id=p_organization_id and ob.status='approved' and v_opening_date is not null and ob.opening_date=v_opening_date
  ...
  account_period as (
    ...
    from public.accounts a left join scoped_facts sf on sf.account_id=a.id
      left join opening_baseline ob on ob.account_id=a.id left join prior_balance_sheet pb on pb.account_id=a.id
    where a.organization_id=p_organization_id and (p_account_id is null or a.id=p_account_id)
    group by a.id,a.code,a.name,a.account_type,a.statement_type,a.statement_section,a.statement_subclassification,ob.debit_minor,ob.credit_minor,pb.debit_minor,pb.credit_minor
  ),
  ...
  select count(*),coalesce(sum(period_debit),0),coalesce(sum(period_credit),0),coalesce(sum(opening_debit),0),coalesce(sum(opening_credit),0),
    coalesce(sum(closing_debit),0),coalesce(sum(closing_credit),0),coalesce(jsonb_agg(to_jsonb(final_rows) order by code),'[]'::jsonb)
  ...
  return jsonb_build_object('organization_id',p_organization_id,'period_id',null,'period_start',p_start_date,'period_end',p_end_date,
    'period_status','date_range','fiscal_year_start',v_fiscal_year_start,'opening_balance_date',v_opening_date,
    'opening_balance_source',case when v_opening_date is null then 'none' else 'opening_balances_approved_only' end,
    'opening_balance_rule','approved opening snapshot only + published balance-sheet movements after snapshot; income-statement accounts have zero opening balance',
