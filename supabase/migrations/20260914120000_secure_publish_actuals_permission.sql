-- Security hardening: publishing actuals is an approval action.
-- Membership alone is not sufficient authorization for authoritative financial publish.
...
begin
  ...
  if p_import_id is null then
    raise exception 'Import is required';
  end if;

  if nullif(trim(p_mapping_version), '') is null then
  ...
  if not public.has_org_permission(v_org, 'approve') then
    raise exception 'Publish approval permission required for this organization';
  end if;

  if v_status = 'rolled_back' then
    raise exception 'Rolled back imports cannot be published';
  end if;
  ...
  loop
    ...
    values(
      ...
      p_import_id, r.source_key, r.payload->>'journal_no', r.payload->>'description',
      v_debit, v_credit
    );
  end loop;
  ...
  values(v_org, p_import_id, trim(p_mapping_version), 'published', v_rows, now())
