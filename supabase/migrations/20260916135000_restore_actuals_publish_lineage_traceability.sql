...
as $function$
declare
 v_user uuid:=auth.uid(); v_org uuid; v_batch uuid; v_rows integer; v_bad integer; v_status text; v_currency text; v_unresolved integer; v_locked integer;
begin
 ...
 select ir.organization_id,fp.id,am.target_account_id,le.target_id,br.target_id,dep.target_id,cc.target_id,rg.target_id,pr.target_id,pj.target_id,v_currency,
 ...
 join public.account_mappings am on am.organization_id=v_org and am.mapping_version=trim(p_mapping_version) and am.source_code=(ir.payload->>'account_code') and am.status='approved'
 ...
 select v_org,null,null,ff.source_row_key,'actual_journal_transaction','financial_fact',ff.id,i.file_name,trim(p_mapping_version),now()
 ...
 values(v_org,p_import_id,v_user,'publish',v_status,'published',jsonb_build_object('row_count',v_rows,'mapping_version',trim(p_mapping_version),'dimensions_resolved',true,'dimension_mapping_source','approved_import_dimension_mappings','lineage_recorded',true));
 return v_batch;
end;
$function$;
