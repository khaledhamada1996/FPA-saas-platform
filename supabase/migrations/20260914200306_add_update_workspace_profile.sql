...
begin
  ...
  update public.organizations set name=btrim(p_name),legal_name=nullif(btrim(coalesce(p_legal_name,'')),''),industry=nullif(btrim(coalesce(p_industry,'')),''),city=nullif(btrim(coalesce(p_city,'')),''),base_currency=p_base_currency,fiscal_year_start_month=p_fiscal_year_start_month,updated_at=now() where id=p_organization_id;
