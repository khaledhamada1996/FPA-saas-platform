...
create function public.calculate_tax_zakat(
  ...
  p_project_id uuid,
  p_regime text,
  p_saudi_ownership_percent numeric,
  p_income_tax_rate numeric,
  p_zakat_rate numeric
)
returns jsonb
language plpgsql
security definer
...
begin
  ...
  return jsonb_build_object(
    'regime',regime,'period_id',p_period_id,'net_income_ytd',coalesce((base->>'net_income_ytd')::bigint,0),
    'profit_before_tax_ytd',ebt,'assets',coalesce((base->>'assets')::bigint,0),'liabilities',coalesce((base->>'liabilities')::bigint,0),'equity',coalesce((base->>'equity')::bigint,0),
    'preliminary_zakat_base',zakat_base,'preliminary_zakat',case when regime in ('zakat','mixed') then zakat else 0 end,
    'taxable_profit_assumption',taxable_profit,'saudi_ownership_percent',ownership,'saudi_taxable_share',saudi_share,'non_saudi_taxable_share',non_saudi_share,
