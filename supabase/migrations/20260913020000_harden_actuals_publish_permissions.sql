...
begin
  ...
  if not public.has_org_permission(v_org,'import') then raise exception 'Not authorized to publish imported actuals'; end if;
  if not public.has_org_permission(v_org,'submit') then raise exception 'Submit permission required to publish imported actuals'; end if;
  ...
  if exists (select 1 from public.import_rows r left join public.account_mappings m on m.organization_id=r.organization_id and m.source_code=(r.payload->>'account_code') and m.status='approved' where r.import_id=p_import_id and m.id is null) then
  ...
  for r in select * from public.import_rows where import_id=p_import_id and validation_status='valid' order by row_number loop
    ...
    if v_period is null then
      ...
      values(v_org,date_trunc('month',v_date)::date,(date_trunc('month',v_date)+interval '1 month - 1 day')::date,'open') returning id into v_period;
    end if;
    select target_account_id into v_account from public.account_mappings where organization_id=v_org and source_code=(r.payload->>'account_code') and status='approved';
  ...
  perform public.write_audit_event(v_org,'actuals.publish','import',p_import_id::text,null,jsonb_build_object('mapping_version',p_mapping_version,'row_count',v_rows,'batch_id',v_batch));
  return v_batch;
end;
$$;
