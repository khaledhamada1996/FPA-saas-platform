-- Secure write boundary for publishing actuals.
-- authenticated users do not receive direct INSERT access to financial_facts
-- or actuals_publish_batches. The publish RPC is the controlled write path.
...
begin
  ...
  insert into public.actuals_publish_batches(
    ...
    v_org, p_import_id, p_mapping_version, 'published', v_rows, now()
...
grant select on table public.financial_facts to authenticated;
