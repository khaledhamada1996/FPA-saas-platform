...
begin
 ...
 with recursive source_rows as (
   select x.value as row,a.id,a.parent_account_id,a.code,a.name
   ...
   select s.*,0::int depth,array[lpad(s.code,20,'0')]::text[] hierarchy_sort from source_rows s where s.parent_account_id is null
   union all
   select c.*,t.depth+1,t.hierarchy_sort||lpad(c.code,20,'0') from tree t join source_rows c on c.parent_account_id=t.id
 ), shaped as (
   select t.*,exists(select 1 from source_rows c where c.parent_account_id=t.id) has_children from tree t
 )
 select coalesce(jsonb_agg(s.row || jsonb_build_object('id',s.id::text,'parent_account_id',s.parent_account_id::text,'depth',s.depth,'has_children',s.has_children,'own_balance',coalesce((s.row->>'period')::bigint,0)) order by s.hierarchy_sort),'[]'::jsonb) into v_accounts from shaped s;
