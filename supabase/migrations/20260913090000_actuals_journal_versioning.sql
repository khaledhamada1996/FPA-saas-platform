...
create index if not exists idx_financial_facts_actual_journal_current
  on public.financial_facts(organization_id, journal_no)
  where fact_type = 'actual';

create index if not exists idx_financial_facts_superseded_by
  on public.financial_facts(superseded_by)
...
begin
  ...
  update public.financial_facts old_fact
     set fact_type='actual_superseded',superseded_by=v_batch_id,superseded_at=now()
   where old_fact.organization_id=v_org and old_fact.fact_type='actual' and old_fact.journal_no is not null
     and old_fact.journal_no in (select distinct r.payload->>'journal_no' from public.import_rows r where r.import_id=p_import_id and nullif(btrim(r.payload->>'journal_no'),'') is not null);
  ...
  select v_org,p_import_id,v_batch_id,'actual',(r.payload->>'date')::date,m.target_account_id,
    round(((coalesce(nullif(r.payload->>'debit','')::numeric,0)-coalesce(nullif(r.payload->>'credit','')::numeric,0))*100))::bigint,
    r.payload->>'description',v_user,r.payload->>'journal_no',
    ...
    coalesce((select max(f.version_no)+1 from public.financial_facts f where f.organization_id=v_org and f.journal_no=r.payload->>'journal_no'),1),
    encode(extensions.digest(convert_to(r.payload::text,'UTF8'),'sha256'),'hex')
  from public.import_rows r join public.account_mappings m on m.organization_id=v_org and m.mapping_version=btrim(p_mapping_version) and m.source_code=(r.payload->>'account_code') and m.status='approved'
  ...
  perform public.write_audit_event(v_org,'actuals.publish','import',p_import_id::text,null,jsonb_build_object('batch_id',v_batch_id,'mapping_version',btrim(p_mapping_version),'row_count',v_rows,'versioning','journal_no_supersedes_previous'));
  return v_batch_id;
end;
$$;
