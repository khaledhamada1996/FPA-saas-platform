-- Reclassify explicit depreciation/amortization expense accounts so EBITDA excludes
-- D&A while EBIT and net income continue to include it.
-- This is intentionally governed by explicit account codes/names, not fuzzy inference.
...
where statement_type = 'income_statement'
  ...
  and statement_subclassification <> 'depreciation_amortization';

-- Deterministic regression helper for financial-statement math.
...
declare
  ...
  v_da bigint := 0;
  v_finance_cost bigint := 0;
  v_other_expense bigint := 0;
  v_tax bigint := 0;
  v_assets bigint := 0;
  ...
  v_ebit bigint := 0;
  v_ebt bigint := 0;
  v_balance_difference bigint := 0;
  v_classified_da_accounts integer := 0;
...
begin
  ...
  where f.organization_id=p_organization_id
    and fp.period_end<=v_period.period_end
    and f.fact_type='actual'
    and f.status='published'
  ...
  select count(*) into v_classified_da_accounts
  ...
  where f.organization_id=p_organization_id and f.financial_period_id=p_period_id and f.fact_type='actual' and f.status='published'
    and a.statement_subclassification='depreciation_amortization';

  return jsonb_build_object(
    'income_statement',jsonb_build_object(
      'revenue',v_revenue,'cogs',v_cogs,'operating_expenses',v_opex,'other_income',v_other_income,
      'depreciation_amortization',v_da,'ebitda',v_ebitda,'ebit',v_ebit,'finance_cost',v_finance_cost,
      'other_expenses',v_other_expense,'ebt',v_ebt,'tax',v_tax,'net_income',v_net_income,
      'ebitda_reconciliation_difference',v_ebitda-(v_ebit+v_da)
    ),
