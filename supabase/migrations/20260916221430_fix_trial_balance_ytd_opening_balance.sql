create or replace function public.get_trial_balance(
  p_organization_id uuid,p_period_id uuid,p_branch_id uuid default null,p_department_id uuid default null,
  p_cost_center_id uuid default null,p_region_id uuid default null,p_product_id uuid default null,
  p_project_id uuid default null,p_account_id uuid default null
) returns jsonb
language plpgsql security definer set search_path to ''
as $function$
declare
  v_user uuid:=auth.uid(); v_period record; v_fiscal_month smallint; v_fiscal_year_start date;
...
begin
  ...
  select fp.id,fp.organization_id,fp.period_start,fp.period_end,fp.status into v_period
  ...
  with scoped_facts as (
    select ff.account_id,ff.financial_period_id,ff.transaction_date,coalesce(ff.debit_minor,0)::bigint debit_minor,coalesce(ff.credit_minor,0)::bigint credit_minor
  ...
  opening_baseline as (
    ...
    where ob.organization_id=p_organization_id and ob.status='approved' and lower(coalesce(oa.statement_type,''))='balance_sheet'
  ...
  prior_balance_sheet as (
    ...
    where sf.transaction_date<v_period.period_start and lower(coalesce(pa.statement_type,''))='balance_sheet' group by sf.account_id
  ),
  prior_income_statement as (
    select sf.account_id,coalesce(sum(sf.debit_minor),0)::bigint debit_minor,coalesce(sum(sf.credit_minor),0)::bigint credit_minor
    from scoped_facts sf join public.accounts pa on pa.id=sf.account_id and pa.organization_id=p_organization_id
    where sf.transaction_date>=v_fiscal_year_start and sf.transaction_date<v_period.period_start and lower(coalesce(pa.statement_type,''))='income_statement' group by sf.account_id
  ),
  account_period as (
    select a.id account_id,a.code,a.name,a.account_type,a.statement_type,a.statement_section,a.statement_subclassification,
      case when lower(coalesce(a.statement_type,''))='balance_sheet' then coalesce(ob.debit_minor,0)-coalesce(ob.credit_minor,0)+coalesce(pb.debit_minor,0)-coalesce(pb.credit_minor,0)
           when lower(coalesce(a.statement_type,''))='income_statement' then coalesce(pi.debit_minor,0)-coalesce(pi.credit_minor,0)
