...
as $$
declare
  v_org uuid; v_status text; v_batch uuid; v_rows integer; v_bad integer; v_mapping_count integer;
  r record; v_date date; v_period uuid; v_account uuid; v_debit bigint; v_credit bigint; v_amount bigint; v_currency text;
begin
  ...
  if exists (
    select 1 from public.import_rows r left join public.account_mappings m on m.organization_id=v_org and m.mapping_version=btrim(p_mapping_version) and m.source_code=(r.payload->>'account_code') and m.status='approved'
    where r.import_id=p_import_id and m.id is null
  ) then raise exception 'Every account must have an approved mapping in the selected mapping version'; end if;
  ...
  for r in select * from public.import_rows where import_id=p_import_id and validation_status='valid' order by row_number loop
    ...
    if v_period is null then
      insert into public.financial_periods(organization_id,period_start,period_end,status) values(v_org,date_trunc('month',v_date)::date,(date_trunc('month',v_date)+interval '1 month - 1 day')::date,'open') returning id into v_period;
    end if;
    ...
    insert into public.financial_facts(organization_id,financial_period_id,account_id,currency,amount_minor,fact_type,source_import_id,source_row_key,journal_no,description,debit_minor,credit_minor)
    values(v_org,v_period,v_account,v_currency,v_amount,'actual',p_import_id,r.source_key,r.payload->>'journal_no',r.payload->>'description',v_debit,v_credit);
  end loop;
  insert into public.actuals_publish_batches(organization_id,import_id,mapping_version,status,row_count,published_at) values(v_org,p_import_id,btrim(p_mapping_version),'published',v_rows,now()) returning id into v_batch;
