-- Financial statements must use published facts only, cumulative balance-sheet balances,
-- fiscal-year-aware YTD income, and governed account subclassification.
...
begin
  ...
  with scoped_facts as (
    ...
    select
      ...
      count(*) filter(where lower(coalesce(statement_type,''))='income_statement' and statement_subclassification is null)::integer unclassified_count
