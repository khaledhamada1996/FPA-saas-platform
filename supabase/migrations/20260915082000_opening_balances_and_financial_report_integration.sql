...
create table if not exists public.opening_balances (
  ...
  branch_id uuid, department_id uuid, cost_center_id uuid, region_id uuid, product_id uuid, project_id uuid,
...
create unique index if not exists opening_balances_org_account on public.opening_balances
  (organization_id, account_id, opening_date, branch_id, department_id, cost_center_id, region_id, product_id, project_id, currency) nulls not distinct;
...
begin
  ...
  return query select ob.id,ob.account_id,a.code,a.name,ob.opening_date,ob.branch_id,ob.department_id,ob.cost_center_id,ob.region_id,ob.product_id,ob.project_id,ob.currency,ob.debit_minor,ob.credit_minor,ob.description,ob.status
    from public.opening_balances ob join public.accounts a on a.id=ob.account_id and a.organization_id=ob.organization_id
    where ob.organization_id=p_organization_id and ob.opening_date=p_opening_date order by a.code,ob.id;
end;
$$;
...
begin
  if not exists (select 1 from public.organizations o where o.id=p_organization_id and o.owner_user_id=auth.uid())
     and not exists (select 1 from public.organization_members om where om.organization_id=p_organization_id and om.user_id=auth.uid() and coalesce(om.role_key,om.role) in ('owner','admin','accountant')) then raise exception 'not authorized'; end if;
...
begin
  if not exists(select 1 from public.organization_members om where om.organization_id=p_organization_id and om.user_id=auth.uid())
     and not exists(select 1 from public.organizations o where o.id=p_organization_id and o.owner_user_id=auth.uid()) then raise exception 'not authorized'; end if;
  select count(*),coalesce(sum(debit_minor),0),coalesce(sum(credit_minor),0) into v_count,v_debit,v_credit
  from public.opening_balances where organization_id=p_organization_id and opening_date=p_opening_date and status in ('approved','locked');
...
create or replace function public.get_financial_statements_date_range_exact(p_organization_id uuid,p_start_date date,p_end_date date,p_journal_no text default null,p_cash_flow_method text default 'indirect')
returns jsonb language plpgsql security definer set search_path = ''
as $$
declare
...
begin
  ...
  if p_journal_no is not null then
    ...
    if v_period_aligned then select public.get_financial_statements_date_range(p_organization_id,p_start_date,p_end_date,null,null,null,null,null,null,null,p_cash_flow_method) into r;
    else select public.get_financial_statements_date_range_exact_raw(p_organization_id,p_start_date,p_end_date,null,p_cash_flow_method) into r; end if;
  end if;
  ...
  select coalesce(jsonb_agg(jsonb_build_object('account_id',x.account_id,'debit',x.debit,'credit',x.credit,'balance',x.debit-x.credit)),'[]'::jsonb) into v_opening
  ...
  for v_account in select value from jsonb_array_elements(v_accounts) loop
    v_code:=v_account->>'code'; select a.id into v_id from public.accounts a where a.organization_id=p_organization_id and a.code=v_code limit 1;
    v_open_debit:=coalesce((select (e->>'debit')::numeric from jsonb_array_elements(v_opening) e where (e->>'account_id')::uuid=v_id limit 1),0); v_open_credit:=coalesce((select (e->>'credit')::numeric from jsonb_array_elements(v_opening) e where (e->>'account_id')::uuid=v_id limit 1),0); v_open_balance:=v_open_debit-v_open_credit;
    v_account:=jsonb_set(jsonb_set(jsonb_set(v_account,'{debit}',to_jsonb(coalesce((v_account->>'debit')::numeric,0)+v_open_debit)),'{credit}',to_jsonb(coalesce((v_account->>'credit')::numeric,0)+v_open_credit)),'{balance}',to_jsonb(coalesce((v_account->>'balance')::numeric,0)+v_open_balance)); v_new_accounts:=v_new_accounts||jsonb_build_array(v_account);
    if lower(coalesce(v_account->>'account_type',''))='asset' then v_total_assets:=v_total_assets+(v_account->>'balance')::numeric; elsif lower(coalesce(v_account->>'account_type',''))='liability' then v_total_liabilities:=v_total_liabilities-(v_account->>'balance')::numeric; elsif lower(coalesce(v_account->>'account_type',''))='equity' then v_total_equity:=v_total_equity-(v_account->>'balance')::numeric; end if;
  end loop;
  ...
  if r ? 'cash_flow' then r:=jsonb_set(r,'{cash_flow,opening_cash}',to_jsonb(coalesce((r->'cash_flow'->>'opening_cash')::numeric,0)+v_opening_cash)); r:=jsonb_set(r,'{cash_flow,closing_cash}',to_jsonb(coalesce((r->'cash_flow'->>'closing_cash')::numeric,0)+v_opening_cash)); r:=jsonb_set(r,'{cash_flow,opening_balance_date}',to_jsonb(v_opening_date)); end if;
  if r ? 'equity_statement' then
    for v_eq_row in select value from jsonb_array_elements(coalesce(r->'equity_statement'->'rows','[]'::jsonb)) loop
      select a.id into v_id from public.accounts a where a.organization_id=p_organization_id and a.code=v_eq_row->>'code' limit 1; v_eq_open:=coalesce((select (e->>'balance')::numeric from jsonb_array_elements(v_opening) e where (e->>'account_id')::uuid=v_id limit 1),0); v_eq_closing:=coalesce((v_eq_row->>'closing')::numeric,0)+v_eq_open; v_eq_row:=jsonb_set(jsonb_set(v_eq_row,'{opening}',to_jsonb(coalesce((v_eq_row->>'opening')::numeric,0)+v_eq_open)),'{closing}',to_jsonb(v_eq_closing)); v_eq_rows:=v_eq_rows||jsonb_build_array(v_eq_row);
    end loop;
