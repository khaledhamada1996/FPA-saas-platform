-- Fix actual journal import validation and row insertion.
-- Keeps the universal journal contract and ERP timestamp support.
...
begin
 ...
 where date_text is null
   or date_text !~ '^[0-9]{4}-[0-9]{2}-[0-9]{2}( [0-9]{2}:[0-9]{2}:[0-9]{2}([.][0-9]+)?)?$'
   or journal_no is null or description is null or account_code is null or account_name is null
 ...
 with parsed as (
   select elem->>'journal_no' journal_no,(elem->>'debit')::numeric debit,(elem->>'credit')::numeric credit
   from jsonb_array_elements(p_rows) as x(elem)),
 unbalanced as (select journal_no from parsed group by journal_no having abs(sum(debit)-sum(credit))>0.005)
 ...
 from jsonb_array_elements(p_rows) with ordinality as x(elem,ordinality);
