...
const RESOURCES=["accounts","journal_entries"] as const;
...
Deno.serve(async(req)=>{
  ...
  try{
    ...
    if(catalog?.connector_key!=="qoyod") throw new Error("ONLY_QOYOD_IS_EXECUTABLE_IN_THIS_STAGE");
    ...
    if(!apiKey) throw new Error("CREDENTIAL_NOT_FOUND");

    async function get(resource:string,page=1){
      const u=new URL(`${QOYOD_BASE}/${resource}`);
      u.searchParams.set("page",String(page));
      u.searchParams.set("per_page","100");
      const r=await fetch(u,{method:"GET",headers:{"API-KEY":apiKey,"Accept":"application/json"}});
      const t=await r.text();
      let p:unknown;
      try{p=JSON.parse(t)}catch{p={raw:t.slice(0,2000)}}
      if(!r.ok) throw new Error(`QOYOD_${r.status}_${resource}`);
      return p;
    }

    if(mode==="test"){
      const a=await get("accounts",1);
      return new Response(JSON.stringify({ok:true,provider:"qoyod",mode,accounts_sample_count:pickRows(a,"accounts").length,read_only:true}),{status:200,headers:JSON_HEADERS});
    }
    ...
    try{
      for(const resource of RESOURCES){
        for(let page=1;page<=100;page++){
          const payload=await get(resource,page),rows=pickRows(payload,resource);
      ...
      return new Response(JSON.stringify({ok:status!=="failed",provider:"qoyod",mode,run_id:run.id,status,received,accepted,updated,errors,read_only:true,idempotent:true}),{status:status==="failed"?502:200,headers:JSON_HEADERS});
    }catch(e){
