...
Deno.serve(async(req)=>{
 ...
 try{
  ...
  for(const r of raw??[]){
   ...
   if(r.source_entity_type==="journal_entries"){
    const list=Array.isArray(p.journal_entries)?p.journal_entries:Array.isArray(p.entries)?p.entries:Array.isArray(p.transactions)?p.transactions:[p];
    for(let i=0;i<list.length;i++){
     const o=obj(list[i]),a=obj(pick(o,["account","account_data","account_info"]));
     const date=dt(pick(o,["date","transaction_date","entry_date","journal_date","created_at"]));
     const debit=money(pick(o,["debit","debit_amount","debit_value","debit_minor"])),credit=money(pick(o,["credit","credit_amount","credit_value","credit_minor"]));
     const aid=pick(a,["id","account_id"])??pick(o,["account_id","account_code","code"]); const missing:string[]=[];
     if(!date)missing.push("transaction_date");if(!aid)missing.push("account");if(debit===0&&credit===0)missing.push("amount");
     out.push({organization_id:org,data_source_id:run.data_source_id,sync_run_id:runId,source_entity_type:"journal_transaction",source_record_key:list.length===1?r.source_record_key:r.source_record_key+":"+String(i+1),transaction_date:date,journal_no:String(pick(o,["journal_no","journal_number","entry_number","number","reference"])??"")||null,description:String(pick(o,["description","memo","note","narration"])??"")||null,account_external_id:String(aid??"")||null,account_code:String(pick(a,["code","account_code"])??pick(o,["account_code","code"])??"")||null,account_name:String(pick(a,["name","account_name"])??pick(o,["account_name","name"])??"")||null,debit_minor:debit,credit_minor:credit,currency:String(pick(o,["currency","currency_code"])??"SAR").slice(0,3).toUpperCase(),dimensions:{branch:pick(o,["branch","branch_name","branch_id"]),department:pick(o,["department","department_name","department_id"]),cost_center:pick(o,["cost_center","cost_center_id"]),region:pick(o,["region","region_name","region_id"]),product:pick(o,["product","product_id"]),project:pick(o,["project","project_id"])},source_payload:r.payload,normalization_status:missing.length?"needs_review":"normalized",normalization_message:missing.length?"بيانات تحتاج مراجعة: "+missing.join(", "):null});
    }
   }else if(r.source_entity_type==="sales"){
    const o=obj(p.data??p),date=dt(pick(o,["date","sale_date","created_at"])),total=money(pick(o,["grand_total","total","amount"])),tax=money(pick(o,["total_tax","tax"]));
    const missing:string[]=[];if(!date)missing.push("transaction_date");if(total===0)missing.push("amount");missing.push("account_mapping");
    out.push({organization_id:org,data_source_id:run.data_source_id,sync_run_id:runId,source_entity_type:"journal_transaction",source_record_key:r.source_record_key,transaction_date:date,journal_no:String(pick(o,["reference_no","reference","id"])??"")||null,description:String(pick(o,["customer","notes"])??"مبيعات Smart Life")||"مبيعات Smart Life",account_external_id:null,account_code:null,account_name:null,debit_minor:total,credit_minor:0,currency:"SAR",dimensions:{customer:pick(o,["customer_id","customer"]),warehouse:pick(o,["warehouse_id"]),biller:pick(o,["biller_id","biller"])},source_payload:r.payload,normalization_status:"needs_review",normalization_message:"مبيعات تشغيلية مستوردة؛ لا تُنشر كقيد محاسبي قبل تحديد الحسابات المقابلة والضريبة من مصدر محاسبي معتمد."});
   }
  }
